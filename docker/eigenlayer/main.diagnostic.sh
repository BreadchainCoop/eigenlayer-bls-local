#!/bin/sh
# DIAGNOSTIC VERSION: Outputs full errors instead of suppressing them
...
# DIAGNOSTIC: Run middleware deployment script with full output
echo "=== Running IncredibleSquaringDeployer.s.sol ==="
cd bls-middleware/contracts
MIDDLEWARE_OUTPUT=$(forge script script/IncredibleSquaringDeployer.s.sol --rpc-url $RPC_URL --skip src/libraries/BN256G2.sol --optimize --broadcast --private-key $PRIVATE_KEY 2>&1)
MIDDLEWARE_EXIT_CODE=$?
if [ $MIDDLEWARE_EXIT_CODE -ne 0 ]; then
    echo "=== MIDDLEWARE DEPLOYMENT FAILED ==="
    echo "Exit code: $MIDDLEWARE_EXIT_CODE"
    echo "=== FULL OUTPUT ==="
    echo "$MIDDLEWARE_OUTPUT"
    echo "=== END OF OUTPUT ==="
    exit 1
fi
echo "Middleware deployment succeeded"
cd /

cp bls-middleware/contracts/script/deployments/incredible-squaring/$chain_id.json ~/.nodes/avs_deploy.json
cp bls-middleware/contracts/script/deployments/incredible-squaring/$chain_id.json bls-middleware/contracts/avs_deploy.json
...
# DIAGNOSTIC: Run BLSSigCheck deployment with full output
echo "=== Running DeployBLSSigCheck.s.sol ==="
BLS_SIG_OUTPUT=$(forge script script/DeployBLSSigCheck.s.sol:DeployBLSSigCheckScript \
       --rpc-url "$RPC_URL"         \
       --private-key "$PRIVATE_KEY" \
       --broadcast 2>&1)
BLS_SIG_EXIT_CODE=$?
if [ $BLS_SIG_EXIT_CODE -ne 0 ]; then
    echo "=== BLSSigCheck DEPLOYMENT FAILED ==="
    echo "Exit code: $BLS_SIG_EXIT_CODE"
    echo "=== FULL OUTPUT ==="
    echo "$BLS_SIG_OUTPUT"
    echo "=== END OF OUTPUT ==="
    exit 1
...
# DIAGNOSTIC: Run Counter deployment with full output
echo "=== Running Counter.s.sol ==="
COUNTER_OUTPUT=$(forge script script/Counter.s.sol:CounterScript \
       --rpc-url "$RPC_URL"         \
       --private-key "$PRIVATE_KEY" \
       --broadcast 2>&1)
COUNTER_EXIT_CODE=$?
if [ $COUNTER_EXIT_CODE -ne 0 ]; then
    echo "=== Counter DEPLOYMENT FAILED ==="
    echo "Exit code: $COUNTER_EXIT_CODE"
    echo "=== FULL OUTPUT ==="
    echo "$COUNTER_OUTPUT"
    echo "=== END OF OUTPUT ==="
    exit 1
...
# DIAGNOSTIC: Run UAMPermissions with full output
echo "=== Running UAMPermissions.s.sol ==="
UAM_OUTPUT=$(forge script script/UAMPermissions.s.sol --rpc-url $RPC_URL --broadcast --private-key $PRIVATE_KEY 2>&1)
UAM_EXIT_CODE=$?
if [ $UAM_EXIT_CODE -ne 0 ]; then
    echo "=== UAMPermissions FAILED ==="
    echo "Exit code: $UAM_EXIT_CODE"
    echo "=== FULL OUTPUT ==="
    echo "$UAM_OUTPUT"
    echo "=== END OF OUTPUT ==="
    exit 1
fi
echo "UAMPermissions completed"

# DIAGNOSTIC: Run SetupMiddleware with full output
echo "=== Running SetupMiddleware.s.sol ==="
SETUP_OUTPUT=$(forge script script/SetupMiddleware.s.sol --rpc-url $RPC_URL --broadcast --private-key $PRIVATE_KEY 2>&1)
SETUP_EXIT_CODE=$?
if [ $SETUP_EXIT_CODE -ne 0 ]; then
    echo "=== SetupMiddleware FAILED ==="
...
echo "SetupMiddleware completed"
...
for i in $(seq 1 $num_accounts); do
    ...
    # DIAGNOSTIC: Run RegisterOperator with full output (already not suppressed in original)
    forge script script/RegisterOperator.s.sol --rpc-url $RPC_URL --broadcast --private-key $OPERATOR_PRIVATE_KEY --isolate --slow --skip-simulation
    if [ $? -ne 0 ]; then
