...
# Amount to mint and deposit into the LST strategy. Must meet the quorum minimumStake.
# Override via MINIMUM_STAKE env var (set by the Helm chart). Defaults to 0.0001 ETH.
STAKE_AMOUNT=${MINIMUM_STAKE:-100000000000000}
...
if [ "$ENVIRONMENT" = "TESTNET" ]; then
    # STAKE_AMOUNT + 0.01 ETH for gas
    run_cast "fund operator $ADDRESS" s $ADDRESS --value $((STAKE_AMOUNT + 10000000000000000)) --private-key "$FUNDED_KEY" -r "$RPC_URL"
    if [ $? -ne 0 ]; then
